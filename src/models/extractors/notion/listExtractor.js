...
export class ListExtractor extends BaseExtractor {
   ...
   * @param {Element} element - Element to check
   * @returns {boolean} - True if element is an ordered list item
   */
  isOrderedListItem(element) {
   ...
   * @param {Element[]} listElements - Array of elements that form a list
   * @param {boolean} isOrdered - Whether this is an ordered list
   * @returns {string} - Markdown list with proper formatting
   */
  processList(listElements, isOrdered = false) {
   ...
   * @param {Element} startElement - First element to check
   * @param {Element|null} endElement - Last element to check or null
   * @returns {Object[]} - Array of list groups with type and elements
   */
  findListGroups(startElement, endElement = null) {
    const listGroups = [];
