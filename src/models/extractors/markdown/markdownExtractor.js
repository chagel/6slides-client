...
export class MarkdownExtractor extends BaseExtractor {
   ...
   * @returns {Element|null} - The markdown container or null
   */
   ...
   * @returns {Object[]} - Array of slide objects
   */
  extractFromRenderedMarkdown(container) {
    try {
   ...
   * @param {Element} currentHeading - Current heading element
   * @param {Element} nextHeading - Next heading element or null
   * @param {Element} container - Parent container
   * @returns {string} - Markdown content
   */
  extractContentBetweenHeadings(currentHeading, nextHeading, container) {
    const markdownParts = [];
    ...
    while (currentElement && 
      ...
      if (tagName === 'h2') {
        ...
        // Images
        const img = tagName === 'img' ? currentElement : currentElement.querySelector('img');
        if (img && img.src) {
   ...
   * @returns {Object[]} - Array of slide objects
   */
  extractFromRawMarkdown() {
    try {
      // Get raw markdown content from the page
      const rawMarkdown = this.document.body.innerText;
   ...
   * @returns {Object[]} - Array of slide objects
   */
  parseAlternativeMarkdownFormat(rawMarkdown) {
